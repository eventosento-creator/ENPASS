alter table public.payments add column if not exists refunded_amount bigint not null default 0 check (refunded_amount >= 0);

drop function if exists public.process_payment_update(text, text, public.payment_status, text, text, bigint, char, bigint, bigint, timestamptz);
...
create function public.process_payment_update(
  ...
  target_refunded_amount bigint default 0
)
returns text
language plpgsql
security definer
...
declare
  ...
  refund_delta bigint := 0;
  refund_ledger bigint := 0;
...
begin
  ...
  set provider_payment_id = coalesce(provider_payment_id, target_provider_payment_id),
      ...
      refunded_amount = case when target_status in ('partially_refunded', 'refunded') then greatest(refunded_amount, coalesce(target_refunded_amount, 0)) else refunded_amount end
  where id = payment_row.id;
  if target_status = 'refunded' and order_row.status = 'paid' then
    ...
    select coalesce(sum(amount), 0) into already_reversed from public.ledger_movements
    where payment_id = payment_row.id and movement_type = 'refund';
    refund_ledger := order_row.subtotal_amount + already_reversed;
    if refund_ledger > 0 then
      insert into public.ledger_movements (organization_id, movement_type, amount, currency, order_id, payment_id, event_id, description)
      values (payment_row.organization_id, 'refund', -refund_ledger, order_row.currency, order_row.id, payment_row.id, order_row.event_id, 'Reembolso de venta');
    end if;
  elsif target_status = 'partially_refunded' and order_row.status = 'paid' then
    -- Tickets stay valid (we cannot know which ones were refunded); the payment is flagged
    -- requires_action so the producer/admin resolves them. Only the ledger moves, proportionally
    -- to the newly refunded amount, so webhook retries with the same amount add nothing.
    refund_delta := coalesce(target_refunded_amount, 0) - payment_row.refunded_amount;
    if refund_delta > 0 and payment_row.gross_amount > 0 and order_row.subtotal_amount > 0 then
      select coalesce(sum(amount), 0) into already_reversed from public.ledger_movements
      where payment_id = payment_row.id and movement_type = 'refund';
      refund_ledger := least(round(refund_delta::numeric * order_row.subtotal_amount / payment_row.gross_amount)::bigint, order_row.subtotal_amount + already_reversed);
      if refund_ledger > 0 then
        insert into public.ledger_movements (organization_id, movement_type, amount, currency, order_id, payment_id, event_id, description)
        values (payment_row.organization_id, 'refund', -refund_ledger, order_row.currency, order_row.id, payment_row.id, order_row.event_id, 'Reembolso parcial');
      end if;
    end if;
...
revoke all on function public.process_payment_update(text, text, public.payment_status, text, text, bigint, char, bigint, bigint, timestamptz, bigint) from public, anon, authenticated;
grant execute on function public.process_payment_update(text, text, public.payment_status, text, text, bigint, char, bigint, bigint, timestamptz, bigint) to service_role;
