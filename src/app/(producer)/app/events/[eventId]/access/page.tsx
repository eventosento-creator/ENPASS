...
export default async function EventAccessPage({ params }: { params: Promise<{ eventId: string }> }) {
  ...
  return <>
    ...
    <section className="mt-8 grid grid-cols-2 gap-3 xl:grid-cols-5">
      <StatCard icon={UsersRound} tone="neutral" label="Ingresos hoy" value={String(metrics.entries_today)}/>
      <StatCard icon={CheckCircle2} tone="emerald" label="Lecturas válidas" value={String(metrics.valid_scans_today)}/>
      <StatCard icon={Clock3} tone="blue" label="Ya utilizadas" value={String(metrics.duplicate_scans_today)}/>
      <StatCard icon={XCircle} tone="red" label="Rechazadas" value={String(metrics.rejected_scans_today)}/>
      <StatCard icon={Smartphone} tone="violet" label="Dispositivos activos" value={String(metrics.active_devices)}/>
    </section>
