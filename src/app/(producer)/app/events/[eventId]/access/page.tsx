...
export default async function EventAccessPage({ params }: { params: Promise<{ eventId: string }> }) {
  ...
  const [eventResult, ticketTypesResult, gatesResult, rulesResult, authorizationsResult, sessionsResult, metricsResult, recentResult] = await Promise.all([
  ...
  return <>
    ...
    <section className="mt-12"><div><p className="eyebrow">Puertas y reglas</p><h2 className="section-title mt-2">Por dónde entra cada ticket</h2></div><div className="mt-5 grid items-start gap-4 lg:grid-cols-2">{gates.map((gate) => <GateEditor gate={gate} rules={rules} ticketTypes={ticketTypesResult.data ?? []} key={gate.id}/>) }<CreateGateForm eventId={event.id} ticketTypes={ticketTypesResult.data ?? []}/></div></section>
