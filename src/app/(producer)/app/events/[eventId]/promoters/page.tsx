...
import { ArrowUpRight, Eye, Link2, Ticket, UsersRound, WalletCards } from "lucide-react";
...
export default async function EventPromotersPage({ params }: { params: Promise<{ eventId: string }> }) {
  ...
  const { data: metrics, error } = await supabase.rpc("get_event_promoter_metrics", { target_event: eventId });
  if (error) throw new Error("No pudimos cargar los RRPP.");
  const rows = metrics ?? [];
  const totals = rows.reduce((result, row) => ({
    tickets: result.tickets + row.tickets_sold,
    revenue: result.revenue + row.ticket_revenue,
