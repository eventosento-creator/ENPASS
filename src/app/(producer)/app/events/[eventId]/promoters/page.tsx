...
import { Armchair, ArrowUpRight, Eye, Link2, Share2, Ticket, TrendingUp, UsersRound, WalletCards } from "lucide-react";
...
export default async function EventPromotersPage({ params }: { params: Promise<{ eventId: string }> }) {
  ...
  return <>
    ...
    {rows.length > 0 && <section className={`mt-6 grid gap-3 sm:grid-cols-2 ${totals.tables > 0 ? "xl:grid-cols-4" : "xl:grid-cols-3"}`}>
      <StatCard icon={Ticket} tone="blue" label="Entradas vía RRPP" value={String(totals.tickets)}/>
      {totals.tables > 0 && <StatCard icon={Armchair} tone="violet" label="Mesas vía RRPP" value={String(totals.tables)}/>}
      <StatCard icon={ArrowUpRight} tone="emerald" label="Ventas atribuidas" value={formatMoney(totals.revenue)}/>
      <StatCard icon={WalletCards} tone="amber" label="Comisión confirmada" value={formatMoney(totals.commission)}/>
    </section>}
    {rows.length ? <section className="mt-8"><div className="flex items-center justify-between gap-4"><div><p className="eyebrow">Ranking</p><h2 className="section-title mt-2">Por ventas atribuidas</h2></div><span className="text-xs font-semibold text-neutral-600">{rows.length} RRPP</span></div><div className="mt-4 grid gap-3">{rows.map((row, index) => <Link href={`/app/events/${eventId}/promoters/${row.event_promoter_id}`} className="card card-interactive grid gap-5 p-5 sm:grid-cols-[auto_1fr_auto] sm:items-center" key={row.event_promoter_id}><span className="grid size-10 place-items-center rounded-full bg-white/[.05] text-sm font-black text-neutral-500">{index + 1}</span><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h3 className="truncate text-lg font-black">{row.display_name}</h3>{row.status === "inactive" && <span className="rounded-full bg-white/[.05] px-2 py-1 text-[10px] font-black uppercase tracking-wider text-neutral-600">Inactivo</span>}</div><p className="mt-1 flex items-center gap-1.5 text-xs text-neutral-600"><Link2 size={13}/>/e/{event.slug}/{row.public_slug}</p><div className={`mt-4 grid gap-3 ${row.tables_sold > 0 ? "grid-cols-4" : "grid-cols-3"}`}><SmallMetric label="Entradas" value={String(row.tickets_sold)}/>{row.tables_sold > 0 && <SmallMetric label="Mesas" value={String(row.tables_sold)}/>}<SmallMetric label="Vendido" value={formatMoney(row.total_revenue, row.currency)}/><SmallMetric label="Comisión" value={formatMoney(row.confirmed_commission, row.currency)}/></div></div><div className="flex items-center justify-between border-t border-white/[.07] pt-4 text-xs text-neutral-500 sm:block sm:border-0 sm:pt-0 sm:text-right"><span className="inline-flex items-center gap-1.5"><Eye size={14}/>{row.visits} visitas</span><p className="mt-1">{conversionLabel(row.paid_orders, row.visits)}</p></div></Link>)}</div></section> : <div className="mt-8"><EmptyState icon={UsersRound} title="Todavía no agregaste RRPP" description="Dales un link propio y seguí sus ventas automáticamente." action={<AddPromoterDrawer eventId={eventId}/>}/></div>}
    <div className="mt-8"><HowItWorksCard title="¿Cómo funciona?" steps={[
      { icon: UsersRound, title: "Crear RRPP", description: "Agregá un representante y generá su link único." },
      { icon: Share2, title: "Compartir link", description: "Tu RRPP comparte su link en redes, Instagram, WhatsApp." },
      { icon: TrendingUp, title: "Seguir ventas", description: "Vas a ver en tiempo real las ventas atribuidas a cada RRPP." },
      { icon: WalletCards, title: "Liquidar comisión", description: "La comisión se congela al confirmar cada pago y podés liquidarla luego." },
    ]}/></div>
  </>;
}
