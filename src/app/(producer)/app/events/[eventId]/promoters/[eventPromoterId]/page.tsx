...
import { CommissionRuleForm, EditPromoterForm, PromoterInviteForm } from "@/modules/promoters/ui/promoter-forms";
...
import type { EventPromoter, Json, Promoter, PromoterCommissionRule, TicketType } from "@/shared/database/types";
...
export default async function EventPromoterDetailPage({ params }: { params: Promise<{ eventId: string; eventPromoterId: string }> }) {
  ...
  const [{ data: event }, { data: relationData }, { data: detailData }] = await Promise.all([
  ...
  const [{ data: promoterData }, { data: ruleData }, { data: ticketTypeData }] = await Promise.all([
  ...
  const breakdown = asBreakdown(detail.ticket_breakdown);
  ...
  return <>
    ...
    <section className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Metric icon={<Ticket size={17}/>} label="Entradas" value={String(detail.tickets_sold)}/><Metric icon={<Link2 size={17}/>} label="Vendido" value={formatMoney(detail.ticket_revenue, detail.currency)}/><Metric icon={<WalletCards size={17}/>} label="Comisión" value={formatMoney(detail.confirmed_commission, detail.currency)}/><Metric icon={<Eye size={17}/>} label="Visitas" value={String(detail.visits)}/></section>
    ...
    <section className="mt-8 grid items-start gap-5 xl:grid-cols-2"><EditPromoterForm eventId={eventId} relation={relation} promoter={promoter}/><CommissionRuleForm eventId={eventId} relationId={relation.id} ticketTypes={ticketTypes} rules={rules}/></section>
    ...
    <section className="mt-10 grid items-start gap-8 lg:grid-cols-2"><div><p className="eyebrow">Desglose</p><h2 className="section-title mt-2">Entradas vendidas</h2>{breakdown.length ? <div className="mt-4 divide-y divide-white/[.07] border-y border-white/[.07]">{breakdown.map((item) => <div className="flex items-center justify-between gap-4 py-4" key={item.name}><span className="text-sm text-neutral-400">{item.name}</span><strong>{item.quantity}</strong></div>)}</div> : <p className="mt-4 text-sm text-neutral-600">Todavía no hay ventas pagadas.</p>}</div><div><p className="eyebrow">Actividad</p><h2 className="section-title mt-2">Últimas ventas</h2>{recentSales.length ? <div className="mt-4 grid gap-3">{recentSales.map((sale) => <article className="card p-4" key={`${sale.created_at}-${sale.items}`}><div className="flex items-start justify-between gap-4"><div><p className="font-bold">{sale.items}</p><p className="mt-1 text-xs text-neutral-600">{relativeDate(sale.created_at)}</p></div><div className="text-right"><p className="font-black">{formatMoney(sale.ticket_revenue, detail.currency)}</p><p className="mt-1 text-xs text-neutral-600">{sale.quantity} {sale.quantity === 1 ? "entrada" : "entradas"}</p></div></div></article>)}</div> : <p className="mt-4 text-sm text-neutral-600">Las ventas confirmadas aparecerán acá sin datos del comprador.</p>}</div></section>
  </>;
}
