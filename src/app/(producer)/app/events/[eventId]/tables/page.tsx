...
export default async function EventTablesPage({ params }: { params: Promise<{ eventId: string }> }) {
  ...
  return <><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="eyebrow">{event.name}</p><h1 className="mt-2 flex items-center gap-3 text-4xl font-black tracking-[-.05em]"><Armchair className="text-[var(--accent)]"/>Mesas</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-neutral-500">Inventario físico, reserva exclusiva y un único QR grupal por mesa.</p></div></div><EventSectionNav eventId={eventId} active="tables" capabilities={capabilities}/><section className="mt-7"><TableManagement eventId={eventId} zones={zones ?? []} tables={managedTables} gates={gates ?? []} editable={event.status === "draft" || event.status === "published"}/></section></>;
}
