...
import { LinkTicketsCard } from "@/modules/events/ui/link-tickets-card";

export default async function EventTicketsPage({ params, searchParams }: { params: Promise<{ eventId: string }>; searchParams: Promise<{ delivery?: string; linknotice?: string; linkerror?: string }> }) {
  ...
  const linkTypes = (ticketTypes ?? []).filter((type) => type.link_only);
  const regularTypes = (ticketTypes ?? []).filter((type) => !type.link_only);
  const { data: linkHolds } = linkTypes.length ? await supabase.from("ticket_holds").select("ticket_type_id, quantity").in("ticket_type_id", linkTypes.map((type) => type.id)).eq("status", "consumed") : { data: [] };
  const linkSold: Record<string, number> = {};
  for (const hold of linkHolds ?? []) linkSold[hold.ticket_type_id] = (linkSold[hold.ticket_type_id] ?? 0) + hold.quantity;
  ...
  return <>
    ...
    {isManager && !["finished", "cancelled"].includes(event.status) && <div className="mt-8"><LinkTicketsCard eventId={event.id} slug={event.slug} siteUrl={siteUrl} ticketTypes={linkTypes} sold={linkSold} notice={query.linknotice} error={query.linkerror}/></div>}
    <div className="mt-8 grid items-start gap-6 xl:grid-cols-[380px_1fr]">
      <div className="card p-5 sm:p-7"><h2 className="section-title">Tipos de entrada</h2><div className="mt-4"><TicketTypeEditor organizationId={event.organization_id} eventId={event.id} ticketTypes={regularTypes} editable={!["finished", "cancelled"].includes(event.status)}/></div></div>
