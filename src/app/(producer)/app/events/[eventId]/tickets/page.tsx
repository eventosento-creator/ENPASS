...
export default async function EventTicketsPage({ params, searchParams }: { params: Promise<{ eventId: string }>; searchParams: Promise<{ delivery?: string }> }) {
  ...
  return <>
    ...
    <div className="mt-8 grid items-start gap-6 xl:grid-cols-[380px_1fr]">
      <div className="card p-5 sm:p-7"><h2 className="section-title">Tipos de entrada</h2><div className="mt-4"><TicketTypeEditor organizationId={event.organization_id} eventId={event.id} ticketTypes={ticketTypes ?? []} editable={!["finished", "cancelled"].includes(event.status)}/></div></div>
