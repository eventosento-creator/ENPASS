...
export default async function NewEventPage({ searchParams }: { searchParams: Promise<{ step?: string; event?: string; profile?: string }> }) {
  ...
  if (step === 1) return <WizardShell step={1} title="Nueva fecha" description="¿Qué estás organizando?"><EventForm organizationId={org.id} venues={venues ?? []} initialProfile={query.profile}/></WizardShell>;
