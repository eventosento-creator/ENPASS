...
export default async function ProducerLayout({ children }: { children: React.ReactNode }) {
  ...
  return <div className="min-h-screen pb-24 md:pb-0 md:pl-60">
    <aside className="fixed inset-y-0 left-0 hidden w-60 border-r border-white/[.07] bg-[#0c0c0e] p-5 md:flex md:flex-col">
      ...
      <div className="mt-auto grid gap-1"><Nav href="/app/settings" icon={<Settings size={17}/>} label="Configuración"/><form action={logout}><button className="flex w-full items-center gap-3 rounded-xl px-3 py-3 text-sm text-neutral-600 hover:text-white"><LogOut size={16}/> Salir</button></form></div>
    </aside>
    ...
    <nav className="fixed inset-x-0 bottom-0 z-20 grid grid-cols-3 border-t border-white/[.08] bg-[#0c0c0e]/95 p-2 backdrop-blur md:hidden"><MobileNav href="/app" icon={<Home/>} label="Inicio"/><MobileNav href="/app/events" icon={<CalendarDays/>} label="Eventos"/><MobileNav href="/app/settings" icon={<Settings/>} label="Ajustes"/></nav>
  </div>;
}
