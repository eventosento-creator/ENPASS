...
export default async function ProducerLayout({ children }: { children: React.ReactNode }) {
  ...
  const [organization, admin, adminViewOrgId] = await Promise.all([getCurrentOrganization(), isPlatformAdmin(), getAdminViewOrgId()]);
  ...
  return <div className="min-h-screen pb-24 md:pb-0 md:pl-60">
    ...
    <header className={`flex items-center justify-between gap-3 border-b border-white/[.07] px-4 py-3 md:hidden ${impersonating ? "mt-9" : ""}`}><Link href="/app"><EnpassLogo compact/></Link><span className="min-w-0 flex-1 truncate text-right text-xs font-bold text-neutral-500">{workspaceLabel}</span><ThemeToggle/></header>
