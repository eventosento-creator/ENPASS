...
export default async function ProducerLayout({ children }: { children: React.ReactNode }) {
  ...
  return <div className="min-h-screen pb-24 md:pb-0 md:pl-60">
    <aside className="fixed inset-y-0 left-0 hidden w-60 border-r border-[var(--border)] bg-[var(--background-soft)] p-5 md:flex md:flex-col">
      ...
      <p className="mt-7 px-3 text-[10px] font-bold uppercase tracking-[.14em] text-neutral-600">Tu espacio</p><p className="mt-1 truncate px-3 text-lg font-black tracking-[-.03em]">{organization?.name ?? "Creá tu evento"}</p><div className="mt-8"><ProducerNavigation/></div>
    ...
    <header className="flex items-center justify-between gap-3 border-b border-white/[.07] px-4 py-3 md:hidden"><Link href="/app"><EnpassLogo compact/></Link><span className="min-w-0 flex-1 truncate text-right text-xs font-bold text-neutral-500">{organization?.name ?? "Tu evento"}</span><ThemeToggle/></header>
    <main className="container-shell py-7 sm:py-10">{children}</main>
    <div className="fixed inset-x-0 bottom-0 z-20 border-t border-white/[.08] bg-[color:var(--background-soft)]/95 px-2 pt-2 pb-[max(.5rem,env(safe-area-inset-bottom))] backdrop-blur md:hidden"><ProducerNavigation mobile/></div>
  </div>;
}
