...
export default async function OrderPage({ params, searchParams }: { params: Promise<{ publicId: string }>; searchParams: Promise<{ paymentError?: string; returned?: string }> }) {
  ...
  const hasTables = items.some((item) => item.item_type === "table");
  ...
  if (paid || (refunded && tickets.length > 0)) return <main className="container-shell min-h-screen py-6 sm:py-10"><section className="mx-auto w-full max-w-2xl">
    <header className="mb-6 flex items-center justify-between"><Link href="/" className="text-sm font-black tracking-[-.03em]">NIGHTLIFE OS</Link><Link href={"/mis-entradas" as never} className="text-xs font-bold text-neutral-500 hover:text-white">Mis accesos</Link></header>
    <div className="mb-6"><p className="eyebrow">{refunded ? "Compra reembolsada" : "Pago confirmado"}</p><div className="mt-3 flex items-start gap-3"><CheckCircle2 className="mt-1 shrink-0 text-[var(--accent)]" size={28}/><div><h1 className="text-3xl font-black tracking-[-.045em] sm:text-4xl">{refunded ? "Estado de tus accesos" : tickets.length ? "¡Ya tenés tus accesos!" : "Estamos preparando tus accesos"}</h1><p className="mt-2 text-sm leading-6 text-neutral-500">{refunded ? "El pago fue reintegrado y los QR dejaron de ser válidos." : tickets.length ? "Guardá este acceso o recuperalo cuando quieras desde Mis accesos." : "Tu pago está confirmado. No vuelvas a pagar."}</p></div></div></div>
  ...
  return <main className="container-shell grid min-h-screen place-items-center py-6 sm:py-10"><section className="w-full max-w-2xl">
    ...
    <div className="card overflow-hidden"><EventCover src={order.event_cover_url} alt={`Flyer de ${order.event_name}`} className="aspect-[16/7]" priority/><div className="p-5 sm:p-8">
      ...
      <div className="mt-7"><h2 className="text-lg font-bold">{order.event_name}</h2><div className="mt-4 grid gap-3">{items.map((item, index) => <div className="flex justify-between text-sm" key={`${item.name}-${index}`}><span className="text-neutral-400">{item.quantity}× {item.name}</span><span>{formatMoney(item.quantity * item.unit_price_amount, order.currency)}</span></div>)}</div><div className="mt-5 grid gap-2 border-t border-white/[.08] pt-5 text-sm"><Row label={hasTables ? "Subtotal" : "Entradas"} value={formatMoney(order.subtotal_amount, order.currency)}/><Row label="Cargo de servicio" value={formatMoney(order.service_fee_amount, order.currency)}/><Row label="Total" value={formatMoney(order.total_amount, order.currency)} strong/></div></div>
      ...
      {expired && !exceptional && <Link href={`/e/${order.event_slug}`} className="btn btn-secondary mt-7 w-full">Volver a elegir</Link>}
...
function OrderHeadline({ paid, expired, refunded, exceptional, paymentStatus }: { paid: boolean; expired: boolean; refunded: boolean; exceptional: boolean; paymentStatus: import("@/shared/database/types").PaymentStatus | null }) {
  ...
  if (expired) return <><p className="eyebrow">Reserva vencida</p><h1 className="mt-3 text-3xl font-black tracking-[-.045em] sm:text-4xl">La reserva ya no está activa</h1><p className="mt-3 text-sm leading-6 text-neutral-500">La disponibilidad se liberó automáticamente para otros compradores.</p></>;
  ...
  return <><p className="eyebrow">Reserva temporal</p><h1 className="mt-3 text-3xl font-black tracking-[-.045em] sm:text-4xl">Tu selección está reservada</h1><p className="mt-3 text-sm leading-6 text-neutral-500">Completá el pago antes de que termine el tiempo.</p></>;
}
