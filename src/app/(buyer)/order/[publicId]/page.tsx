...
export default async function OrderPage({ params, searchParams }: { params: Promise<{ publicId: string }>; searchParams: Promise<{ paymentError?: string; returned?: string }> }) {
  ...
  const free = order.total_amount === 0;
  ...
  const canPay = order.status === "pending" && !free && order.payment_account_connected && !exceptional;
  ...
  return <main className="container-shell grid min-h-screen place-items-center py-6 sm:py-10"><section className="w-full max-w-2xl">
    ...
    <div className="card overflow-hidden"><EventCover src={order.event_cover_url} alt={`Flyer de ${order.event_name}`} className="aspect-[16/7]" priority/><div className="p-5 sm:p-8">
      ...
      <div className="mt-7"><h2 className="text-lg font-bold">{order.event_name}</h2><div className="mt-4 grid gap-3">{items.map((item, index) => <div className="flex justify-between text-sm" key={`${item.name}-${index}`}><span className="text-neutral-400">{item.quantity}× {item.name}</span><span>{item.unit_price_amount === 0 ? "Gratis" : formatMoney(item.quantity * item.unit_price_amount, order.currency)}</span></div>)}</div><div className="mt-5 grid gap-2 border-t border-white/[.08] pt-5 text-sm"><Row label={hasTables ? "Subtotal" : "Entradas"} value={free ? "Gratis" : formatMoney(order.subtotal_amount, order.currency)}/>{!free && <Row label="Cargo de servicio" value={formatMoney(order.service_fee_amount, order.currency)}/>}<Row label="Total" value={free ? "Gratis" : formatMoney(order.total_amount, order.currency)} strong/></div></div>
