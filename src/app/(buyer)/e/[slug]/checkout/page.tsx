...
type CheckoutItem = { id: string; itemType: "ticket" | "table" | "seat"; name: string; quantity: number; unitPrice: number; serviceFeeBps: number };
...
export default async function CheckoutPage({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams: Promise<{ selection?: string }> }) {
  ...
  if (!event) notFound();
  const [{ data: publicTypes }, { data: publicTables }, { data: publicSeats }, { data: organization, error: feePolicyError }] = await Promise.all([
    supabase.rpc("get_public_ticket_types", { target_event: event.id }),
    supabase.rpc("get_public_event_tables", { target_event: event.id }),
    supabase.rpc("get_public_event_seats", { target_event: event.id }),
    admin.from("organizations").select("service_fee_bps, fee_payer").eq("id", event.organization_id).single(),
  ]);
  ...
  const seats = (publicSeats ?? []) as PublicEventSeat[];
  ...
  const items = selections.flatMap((selection): CheckoutItem[] => {
    ...
    if (selection.item_type === "seat") {
      const seat = seatById.get(selection.item_id);
      if (!seat || seat.availability_status !== "available") return [];
      return [{ id: seat.id, itemType: "seat", name: `${seat.section_name} · ${seat.label}`, quantity: 1, unitPrice: seat.base_price_amount, serviceFeeBps: seat.service_fee_bps }];
    }
