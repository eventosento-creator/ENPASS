...
export default async function CheckoutPage({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams: Promise<{ selection?: string }> }) {
  ...
  return <main className="container-shell min-h-screen py-5 sm:py-12"><header className="flex items-center justify-between"><Link href={`/e/${slug}`} className="inline-flex min-h-11 items-center gap-1 text-sm text-neutral-500 hover:text-white"><ChevronLeft size={17}/>Cambiar selección</Link><Link href="/" className="text-sm font-black tracking-[-.03em]">NIGHTLIFE OS</Link></header><div className="mx-auto mt-7 grid max-w-4xl gap-6 lg:grid-cols-[1fr_330px]"><section className="card p-5 sm:p-8"><p className="eyebrow">Checkout</p><h1 className="mt-3 text-3xl font-black tracking-[-.04em] sm:text-4xl">¿A quién enviamos los accesos?</h1><p className="mb-8 mt-3 text-sm leading-6 text-neutral-500">Pedimos solo lo necesario. Comprás como invitado, sin crear una cuenta.</p><CheckoutForm eventId={event.id} selections={selections} requireDocument={event.require_document}/></section><aside className="card order-first h-fit overflow-hidden lg:order-last"><EventCover src={event.cover_image_url} alt={`Flyer de ${event.name}`} className="aspect-[16/7]" sizes="(max-width: 1024px) 100vw, 330px"/><div className="p-5"><p className="text-[11px] font-black uppercase tracking-[.12em] text-neutral-600">Tu compra</p><h2 className="mt-2 font-bold">{event.name}</h2><div className="mt-4 grid gap-3 border-y border-white/[.07] py-4">{items.map((item) => <div className="flex justify-between gap-4 text-sm" key={`${item.itemType}-${item.id}`}><span className="text-neutral-400">{item.quantity}× {item.name}</span><span className="shrink-0">{formatMoney(item.unitPrice * item.quantity, event.currency)}</span></div>)}</div><div className="mt-4 grid gap-2 text-sm"><SummaryRow label={hasTables ? "Subtotal" : "Entradas"} value={formatMoney(subtotal, event.currency)}/><SummaryRow label="Cargo de servicio" value={formatMoney(serviceFee, event.currency)}/><SummaryRow label="Total" value={formatMoney(total, event.currency)} strong/></div></div></aside></div></main>;
}
