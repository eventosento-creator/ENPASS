...
export default async function CheckoutPage({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams: Promise<{ selection?: string }> }) {
  ...
  if (!event) notFound();
  const [{ data: publicTypes }, { data: publicTables }, { data: publicSeats }, { data: organization, error: feePolicyError }, { data: termsDoc }, { data: refundDoc }] = await Promise.all([
    ...
    supabase.rpc("get_active_legal_document", { target_type: "terms_buyer" }),
    supabase.rpc("get_active_legal_document", { target_type: "refund_policy" }),
  ]);
  if (feePolicyError || !organization) throw new Error("CHECKOUT_FEE_POLICY_UNAVAILABLE");
  if (!termsDoc || !refundDoc) throw new Error("CHECKOUT_LEGAL_DOCUMENTS_UNAVAILABLE");
