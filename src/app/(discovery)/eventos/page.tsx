...
export default async function EventsDiscoveryPage({ searchParams }: { searchParams: Promise<{ city?: string | string[]; when?: string | string[]; category?: string | string[]; q?: string | string[] }> }) {
  ...
  const hasActiveFilters = Boolean(filters.city) || filters.when !== "all" || Boolean(filters.category) || Boolean(filters.q);
  ...
  return <main className="pb-16 pt-6">
    ...
    </> : <EmptyState icon={CalendarX2} title="No encontramos eventos con esos filtros" description="Probá cambiar la búsqueda, la categoría, la fecha o la ciudad." action={<Link href="/eventos" className="btn btn-secondary">Ver todos los eventos</Link>}/>}</section></div>
