...
export function HoldCountdown({ expiresAt, eventSlug, initiallyExpired = false }: { expiresAt: string; eventSlug: string; initiallyExpired?: boolean }) {
  ...
  if (expired) return <div className="status-danger rounded-[1.1rem] p-5"><p className="font-bold">Tu reserva venció.</p><p className="mt-1 text-sm text-neutral-500">Las entradas ya están disponibles para otra persona.</p><Link href={`/e/${eventSlug}`} className="btn btn-secondary mt-4 w-full">Elegir entradas otra vez</Link></div>;
  return <div className="flex items-center justify-between gap-4 rounded-[1.1rem] border border-white/[.08] bg-black/25 p-4"><div className="flex items-center gap-3"><Clock3 className="text-[var(--accent)]" size={19}/><div><p className="text-sm font-bold">Tiempo restante</p><p className="mt-0.5 text-xs text-neutral-600">La reserva se libera al finalizar.</p></div></div><time className="shrink-0 font-mono text-2xl font-black tracking-tight" dateTime={expiresAt}>{countdown}</time></div>;
}
