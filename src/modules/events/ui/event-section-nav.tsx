...
export function EventSectionNav({ eventId, active, capabilities, profile = "nightlife" }: { eventId: string; active: "summary" | "tickets" | "guests" | "promoters" | "tables" | "seatmap" | "access" | "pos" | "boxoffice"; capabilities: EventCapabilities; profile?: EventProfile }) {
  ...
  const visibleItems = items.filter((item) => item.key === "summary" || (item.key === "guests" ? capabilities.tickets || capabilities.tables || capabilities.seatmap : item.key === "boxoffice" ? capabilities.tickets : capabilities[item.key]));
