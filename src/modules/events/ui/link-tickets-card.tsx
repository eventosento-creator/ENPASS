...
export async function LinkTicketsCard({ eventId, slug, siteUrl, ticketTypes, sold, notice, error }: {
  ...
  const items = await Promise.all(ticketTypes.map(async (type) => {
    ...
    return { type, url, qr: await QRCode.toDataURL(url, { errorCorrectionLevel: "M", margin: 1, width: 240 }) };
  }));
  return <div className="card p-5 sm:p-7">
    ...
    <div className="mt-5 grid gap-4">
      {items.map(({ type, url, qr }) => <article key={type.id} className="rounded-[1.15rem] border border-white/[.08] p-4">
        ...
        <div className="mt-3 flex flex-wrap gap-2"><CopyLinkButton value={url}/>
