export function AvailabilityIndicator({ reserved, capacity, compact = false, label = "reservas" }: { reserved: number; capacity: number; compact?: boolean; label?: string }) {
  ...
  return <div>
    <div className="flex items-baseline justify-between gap-3"><p className={compact ? "text-xs text-white/65" : "text-sm text-white/70"}><strong className="text-white">{reserved}</strong> / {capacity} {label}</p><span className="text-[11px] text-white/40">{percentage}%</span></div>
