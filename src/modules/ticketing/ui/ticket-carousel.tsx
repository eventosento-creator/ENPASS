...
export function TicketCarousel({ tickets }: { tickets: TicketPresentation[] }) {
  ...
  return <>
    <section className="ticket-shell touch-pan-y overflow-hidden" onTouchStart={(event) => { touchStart.current = event.touches[0]?.clientX ?? null; }} onTouchEnd={(event) => finishSwipe(event.changedTouches[0]?.clientX ?? 0)}>
      ...
      <div className="p-5 sm:p-7">
        <div className="flex items-start justify-between gap-4">
          <div><p className="eyebrow">{ticket.credentialKind === "table" ? "Tu mesa" : hasMultiple ? "Tus entradas" : "Tu entrada"}</p><h2 className="mt-2 text-3xl font-black tracking-[-.045em]">{ticket.eventName}</h2></div>
    ...
    {hasMultiple && <nav aria-label="Cambiar credencial" className="mt-4 flex items-center justify-between gap-3">
    ...
    {fullscreen && ticket.qrSvg && <div className="fixed inset-0 z-[100] overflow-y-auto bg-white text-black" role="dialog" aria-modal="true" aria-label={`QR grande, credencial ${index + 1} de ${tickets.length}`}>
