...
function ActivationScreen({ onActivated }: { onActivated: (session: ScannerSessionView) => void }) {
  ...
  return <div className="safe-bottom mx-auto flex min-h-dvh w-full max-w-md flex-col px-5 pt-[max(1.75rem,env(safe-area-inset-top))] sm:justify-center">
    <div className="flex items-center gap-2 text-xs font-black tracking-[.17em] text-neutral-500"><ScanLine size={17} className="text-[var(--accent)]"/> ENPASS ACCESS</div>
...
function ActiveScanner({ session, developmentMode, onSessionEnded }: { session: ScannerSessionView; developmentMode: boolean; onSessionEnded: () => void }) {
  ...
  const processPayload = useCallback(async (payload: string) => {
    ...
    if (!isNightlifeQrPayload(payload)) {
      showResult({ result: "invalid", checkin_id: null, ticket_id: null, holder_name: null, ticket_type_name: null, sector: null, short_code: null, used_entries: null, max_entries: null, first_used_at: null, first_used_gate_name: null, valid_from: null, valid_until: null, suggested_gate_name: null, scanned_at: new Date().toISOString(), is_courtesy: null });
      return;
    }
    try {
