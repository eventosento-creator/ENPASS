...
function ActivationScreen({ onActivated }: { onActivated: (session: ScannerSessionView) => void }) {
  ...
  return <div className="mx-auto flex min-h-dvh w-full max-w-md flex-col px-5 py-7 sm:justify-center">
...
function ActiveScanner({ session, developmentMode, onSessionEnded }: { session: ScannerSessionView; developmentMode: boolean; onSessionEnded: () => void }) {
  ...
  const [cameraState, setCameraState] = useState<"starting" | "ready" | "denied" | "error">("starting");
  ...
  return <div className="relative mx-auto flex min-h-dvh w-full max-w-3xl flex-col overflow-hidden bg-[#070708]">
    <header className="z-10 flex items-center justify-between gap-4 border-b border-white/[.08] bg-black/55 px-4 py-3 backdrop-blur"><div className="min-w-0"><p className="truncate text-sm font-black">{session.event_name}</p><p className="mt-0.5 flex items-center gap-1.5 text-[11px] font-bold text-neutral-500"><span className="size-1.5 rounded-full bg-[var(--accent)]"/>{session.gate_name} · {session.permission === "supervisor" ? "Supervisor" : "Scanner"}</p></div><div className="flex items-center gap-2"><span className={`hidden items-center gap-1.5 text-[11px] font-bold sm:flex ${online ? "text-lime-300" : "text-red-300"}`}>{online ? <Wifi size={14}/> : <WifiOff size={14}/>}{online ? "Online" : "Sin red"}</span><button className="grid size-10 place-items-center rounded-xl border border-white/[.08] bg-white/[.04]" onClick={() => setSettingsOpen((value) => !value)} aria-label="Configuración"><Settings2 size={18}/></button></div></header>

    <section className="relative flex min-h-[62dvh] flex-1 items-center justify-center overflow-hidden bg-black"><video ref={videoRef} className="absolute inset-0 size-full object-cover" muted playsInline/><div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_center,transparent_20%,rgb(0_0_0/.36)_78%)]"/>{cameraState === "starting" && <CameraMessage icon={<Camera/>} title="Iniciando cámara" detail="Preparando el lector…"/>}{cameraState === "denied" && <CameraMessage icon={<Camera/>} title="Habilitá la cámara" detail="El scanner necesita permiso para leer entradas." action={<button className="btn btn-secondary mt-5" onClick={() => window.location.reload()}>Reintentar</button>}/>} {cameraState === "error" && <CameraMessage icon={<AlertTriangle/>} title="Cámara no disponible" detail="Revisá el permiso o la conexión e intentá de nuevo." action={<button className="btn btn-secondary mt-5" onClick={() => window.location.reload()}><RotateCcw size={16}/>Reintentar</button>}/>} {!online && <div className="absolute inset-0 z-20 grid place-items-center bg-[#160b0d]/95 p-8 text-center"><div><WifiOff className="mx-auto text-red-300" size={38}/><h2 className="mt-5 text-3xl font-black">SIN CONEXIÓN</h2><p className="mx-auto mt-3 max-w-xs text-sm leading-6 text-red-100/60">La validación está detenida. No se permiten ingresos offline.</p></div></div>}
      {result && (
