...
export function AuthForms({ initialMode = "login", nextPath = "/app", notice, errorNotice }: { initialMode?: AuthMode; nextPath?: string; notice?: string; errorNotice?: string }) {
  ...
  return <div className="card w-full max-w-md p-5 sm:p-8">
    <p className="eyebrow">Productores</p><h1 className="mt-3 text-3xl font-black tracking-[-.04em]">{mode === "login" ? "Entrá a tu panel" : mode === "register" ? "Creá tu cuenta" : mode === "magic" ? "Acceso por email" : "Recuperá tu contraseña"}</h1><p className="mt-3 text-sm leading-6 text-neutral-500">{mode === "login" ? "Administrá tus fechas, ventas y accesos." : mode === "register" ? "Empezá con tu organización y primer evento." : mode === "magic" ? "Te enviamos un enlace seguro para ingresar." : "Recibí un enlace seguro para elegir una contraseña nueva."}</p>
    <form action={formAction} className="mt-7 grid gap-4">
      ...
      {(mode === "login" || mode === "register") && <label className="label">Contraseña<input className="field" name="password" type="password" minLength={8} autoComplete={mode === "login" ? "current-password" : "new-password"} required /></label>}
