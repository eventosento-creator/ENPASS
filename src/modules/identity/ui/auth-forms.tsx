...
export function AuthForms({ initialMode = "login", nextPath = "/app" }: { initialMode?: AuthMode; nextPath?: string }) {
  ...
  return <div className="card w-full max-w-md p-5 sm:p-8">
    <p className="eyebrow">Productores</p><h1 className="mt-3 text-3xl font-black tracking-[-.04em]">{mode === "login" ? "Entrá a tu panel" : mode === "register" ? "Creá tu cuenta" : "Acceso por email"}</h1><p className="mt-3 text-sm leading-6 text-neutral-500">{mode === "login" ? "Administrá tus fechas, ventas y accesos." : mode === "register" ? "Empezá con tu organización y primer evento." : "Te enviamos un enlace seguro para ingresar."}</p>
