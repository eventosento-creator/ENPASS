  ...
  return <header className="sticky top-0 z-30 border-b border-white/[.07] bg-[#090909]/90 backdrop-blur-xl">
    <div className="container-shell flex h-16 items-center justify-between gap-2">
      ...
      <nav aria-label="Navegación principal" className="flex items-center gap-0.5 text-sm sm:gap-1">
        <Link className="hidden rounded-full px-2.5 py-2 text-neutral-400 transition hover:bg-white/[.05] hover:text-white sm:inline-flex sm:px-3" href="/eventos">Comprar entradas</Link>
        ...
        <Link aria-label="Ingresar como productor" className="inline-flex min-h-10 items-center gap-2 rounded-full px-2.5 py-2 text-neutral-400 transition hover:bg-white/[.05] hover:text-white sm:px-3" href="/login"><LogIn aria-hidden size={16}/><span className="hidden md:inline">Ingresar</span></Link>
        <Link className="btn btn-primary min-h-10 px-3 py-2 sm:ml-1 sm:px-3.5" href="/crear-evento/iniciar"><Plus aria-hidden size={16}/><span className="hidden sm:inline">Crear evento</span><span className="sm:hidden">Crear</span></Link>
      </nav>
    </div>
  </header>;
}
